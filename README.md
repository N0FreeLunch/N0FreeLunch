...
- 🌱 I’m currently learning Laravel Test driven design, domain driven design with function composition (JS), ReactJS, Golang, Agile software development
...
4. AWS : EC2(ALB, Spot Instance, Auto scale), Lambda, RDS, cloudfront, cloudwatch, S3, Athena, Route53, SES/SNS, API Gateway, IAM, Elastic Cache, ECS(Fargate, cloudformation), Secret Manager
...
- Docker setting (To define auto webpack building and composer installing process with considering security and maintanace when deploy build system)
...
- Refactored query builder base code to eloquentORM with the concept of specification.
...
- Made batch cancellation APIs and artisan commands. (laravel artisan console, queue, job)
- Improved a batch process create Invoices and Reports using queue/job/triggered by an administrator instead of being solely batched by the scheduler. (laravel job, queue)
- Added batch execution API for QA to test batches. (middleware that resctirct production environment)
...
- Improved account payment logic to use saved information when file is uploaded.
...
- Improved code to verify PCIDSS compliance. (customizing validation)
...
- Created a logging feature that record deprecated syntax. (laravel logging, monolog libray, laravel job)
- Added feature test environment. (pest, postgre/inmemory sqlite, model factory)
- Refactored email sending logic to encapsulate domain constraints. Fixed bug bank account register logic. (TDD)
...
- Script for showing advertise to media web site from AD network. Being published in media site web page advertise area. (vanilla JS, webpack, DOM manipulate)
...
- Admin service and user client service development/management/maintenance (Laravel, VueJS, vue-router, JWT protocol)
...
- To learn modern JS (ES6, ES7, ES8...), functional abstract, 
...
- [Rails](https://github.com/N0FreeLunch/ruby-rails-study)
...
- [Elm](https://github.com/N0FreeLunch/elm-basic), [Scala](https://github.com/N0FreeLunch/scala-start), [Haskell](https://github.com/N0FreeLunch/haskel-practice)
